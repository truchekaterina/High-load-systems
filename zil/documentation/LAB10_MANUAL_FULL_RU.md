...
Нужны **и** `docker compose`, **и** `k6` **на этой же машине**. Если k6 ставите только на **hl11**, этот подраздел **не используйте** — только «**k6 на hl11, Docker на hl07**» выше (на **hl07** отдельно k6 не нужен).

На одной ВМ (например отдельный стенд, где уже есть оба инструмента):
