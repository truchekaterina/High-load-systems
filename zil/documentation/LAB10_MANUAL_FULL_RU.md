...
## 2. Карта ВМ из инфраструктурной таблицы (пример)

| Роль | IP | SSH-порт (пример) |
|------|-----|-------------------|
| **k6** | `10.60.3.8` | `2311` |
| **PostgreSQL** | `10.60.3.9` | `2312` |
| **Harbor (registry)** | `10.60.3.11` | `2313` |
| **k3s master** | `10.60.3.14` | `2316` |
| **k3s worker** | `10.60.3.15` | `2317` |

Для студента в таблице может быть отдельный SSH-порт (например **2307**) и имя БД **hl7** — это согласуется с `DBNAME=hl7` в `registry-tags-lab8-hl7.env`. Хост БД в env-файле должен совпадать с реальным стендом (внутренний DNS или IP ВМ БД).

**Порты сервисов:**

- Основное приложение (**app**) — **8083**.
- **additional** — **8084**.
- Лимиты CPU в `docker-compose.yml` задаются переменными **`APP_CPUS`** и **`ADDITIONAL_CPUS`** на хосте до `docker compose up`, не только комментариями в `.env`.

Если приложение в **k3s**, принцип тот же: `limits.cpu: 500m` против `1000m` для подов **app** и **additional** — уточните у преподавателя, где «эталон» сдачи (compose или k3s).
