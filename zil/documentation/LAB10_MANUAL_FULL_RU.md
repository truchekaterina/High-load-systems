...
**Стенд:** сервисы поднимаются через **`docker compose`** на ВМ приложений (часто **hl07** — «здесь» у вас Docker). **k6** в типичной схеме курса стоит **на другой виртуалке** (например **hl11** или узел нагрузки из таблицы) — матрицу LAB10 и обычные прогоны запускают **с неё**, а на ВМ с Docker держат только контейнеры и SSH-доступ. **Kubernetes / k3s в этом руководстве не рассматриваются** — при появлении такого стенда лимиты CPU и логику деплоя нужно переносить на него отдельно по методичке.
...
Отдельная строка в таблице студента — SSH на **вашу** ВМ приложений (например порт **2307**): с неё выполняются **`docker compose`**, `pull` образов и при необходимости работа с Harbor. **Отдельно** в таблице может быть ВМ под **k6** — нагрузку выполняют с неё, а `docker compose` на ВМ приложений при этом **не дублируют** инструментом k6.
