...
Для графиков в форме LAB8 (**три панели** POST прокси / GET stats по смесям **STATS_SHARE** и парой CPU на каждый `@KafkaListener` concurrency) в репозитории есть **`k6/run-lab13-kafka-proxy-matrix.sh`**: по умолчанию (**`LAB13_MIX_PANELS=0`**) он выполняет только **четыре** прогона из таблицы §0.8 (**`summary_CPU05_conc1.json`** … без суффикса **`_mix`**). Чтобы получить по **три** смеси на каждую ячейку и имена вида **`summary_CPU05_conc1_mix50.json`**, задайте **`LAB13_MIX_PANELS=1`** (итого **12** прогонов k6 с теми же порциями **`STATS_SHARE`**, что в LAB10/LAB8: **`0.05` / `0.5` / `0.95`** → **`_mix05`**, **`_mix50`**, **`_mix95`**). Картинки **`reports-lab13/lab13_latency_vs_cpu.png`** (и **`…_conc1.png`**, **`…_conc2.png`**) строит **`plot_lab13_reports.py`** при наличии полного набора **`*_mix*.json`**. Переменная **`LAB13_PANEL_CONC`** задаёт, какая панель дублируется в **`lab13_latency_vs_cpu.png`** (по умолчанию **`2`**).
...
**Итог по выбранному варианту «прокси + HTTP из k6» (для отчёта):** маленький **отдельный модуль/сервис** в репозитории (**`zil/lab13-kafka-proxy/`**), реализация на **Python**, **Java/Spring Boot** или сочетание с **Docker Compose** по вашему выбору; снаружи — **REST `POST`** (**`/publish`** и т.п.), внутри — **Kafka Producer** в топик эксперимента (**`hl07-lab13`**, **2 партиции**, см. **`registry-tags-lab13-topic.env`**); **k6** делает только **`http.post`** на URL этого сервиса. Удобно поднимать сервис на **ВМ нагрузки** (SSH шлюз **2311**, IP **`10.60.3.8`**) **рядом с уже установленным k6** — без сборки расширяемого k6.
...
2. **Прокси** — маленький процесс (**Python + FastAPI/Flask + `kafka-python`**, или минимальный **Spring Boot** с **`KafkaTemplate`**) принимает **JSON** и публикует его **value** в топик эксперимента (**`hl07-lab13`**) через **Kafka Producer**.
3. **`zil-app`** (**2307**) читает **тот же** топик (**`KAFKA_TOPIC`**, при LAB13 обычно **`hl07-lab13`**) и применяет команды LAB12 (**`KafkaRentalCommandRouter`**).
...
| `KAFKA_TOPIC` | Топик (должен совпадать с **`app`**) | **`hl07-lab13`** (LAB13, 2 партиции); LAB12 по таблице часто **`hl07`** |
...
2. **Переменные окружения** — см. §2.2.3; **`KAFKA_TOPIC`** прокси совпадает с **`KAFKA_TOPIC`** контейнера **`app`** на **2307** (**`hl07-lab13`** при использовании **`registry-tags-lab13-topic.env`**).
...
2. **Consumer** в LAB12 обычно читает топик **`hl07`** из таблицы выдачи; для LAB13 с **2 партициями** по ТЗ переключают **`KAFKA_TOPIC`** на **`hl07-lab13`** (см. **`registry-tags-lab13-topic.env`**), добавляют **batch** и **переключаемость concurrency** через env **`KAFKA_LISTENER_CONCURRENCY`** / **`app.kafka.listener.concurrency`**.
...
mkdir -p reports-lab13
export PROXY_URL=http://127.0.0.1:18080/publish
export KAFKA_TOPIC=hl07-lab13   # как у прокси и у app (§0.6)
...
Документ отражает **LAB13** (**Трюх Екатерина**, SSH **2307**, БД **`hl7`**): экспериментальный топик **`hl07-lab13`** (**2 партиции**), учебный топик из таблицы — **`hl07`** (LAB11–LAB12). После реализации допускается подраздел **«Результаты»** с графиками или ссылкой на архив без секретов.
