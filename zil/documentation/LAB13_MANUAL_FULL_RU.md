...
| § | Раздел |
|---|--------|
| **0** | **Пошаговый план** — **`§0.0.1`** команды по ВМ (Kafka → приложение → k6), **`§0.10`** сценарий показа преподавателю, далее детали SSH и матрица |
| 1 | Выдача, ВМ, топик |
...
### 0.10. Как показать работу LAB13 преподавателю (устная демонстрация по ТЗ)

Имеется в виду **живой** прогон по цепочке из §0.9 и подтверждение пунктов ТЗ: нагрузка **не** бьёт в REST основного приложения для мутаций, а идёт в **Kafka**; консьюмер — **batch**; топик эксперимента — **2 партиции**; матрица **CPU 0.5 и 1.0** (одинаково **app** и **additional**) × **`KAFKA_LISTENER_CONCURRENCY`** **1** и **2**.

**Переключение на ветку LAB13** (на любом клоне — ПК, **2307**, **2311**):
...
**Рекомендуемый порядок экранов для препода:**

| Шаг | Что показать | Где команды |
|-----|----------------|-------------|
| 1 | В кластере Kafka топик **`hl07-lab13`**, в **`describe`** — **`PartitionCount: 2`** | SSH **2314/2315**, §0.5 |
| 2 | В репозитории ветка **`docs/lab13-kafka-k6-batch`**, в коде **`app`** — **batch** `@KafkaListener` и настраиваемый **concurrency** | IDE или `git log -1` на **2307** |
| 3 | Контейнеры **`app`** + **`additional`** с одинаковым CPU (**0.5** или **1.0**) и заданным **`KAFKA_LISTENER_CONCURRENCY`**; в логах **`app`** — подписка на **`hl07-lab13`** | **2307**, §0.4 |
| 4 | Запущенный **REST-прокси** на **2311** (**`POST /publish`** → Kafka) | **2311**, §0.6 |
| 5 | **Один** успешный **`curl`** на прокси (HTTP **200**), затем фрагмент логов **`app`** с обработкой команды из Kafka | §0.7 |
| 6 | Запуск **k6** с **`load-lab13-kafka-proxy.js`** (**запись только через HTTP в прокси**, не в **8083**) и файл **`summary_*.json`** для выбранной ячейки матрицы | **2311**, §0.8 |
| 7 | **Графики** зависимости метрик от **CPU × concurrency** (минимум четыре прогона по таблице §0.8; при полном наборе смесей — **`plot_lab13_reports.py`**) | §0.8, §6 |

**Одной фразой по ТЗ:** нагрузочный профиль изменён так, что **k6 публикует команды в топик через прокси**, основное приложение **читает партиями** из Kafka и пишет в **PostgreSQL**; сравниваются режимы при ограничении CPU и разном **concurrency** при **двух партициях**.

Подробные команды без сокращений — **§0.0.1**. Перечень артефактов к отчёту — **§6**.
