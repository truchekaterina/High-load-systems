...
Пока пункты не готовы — шаги **0.5–0.8** являются проверкой «как должно быть»; curl/k6 упадут без прокси и скрипта.

| № | Что сделать | Где искать пояснения в этом файле |
|---|-------------|-----------------------------------|
...
| 4 | **`zil/k6/load-lab13-kafka-proxy.js`** + переменная **`PROXY_URL`** | §2.2.5, §4.5 |
