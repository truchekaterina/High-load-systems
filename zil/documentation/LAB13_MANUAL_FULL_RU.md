...
Для графиков в форме LAB8 (**три панели** POST прокси / GET stats по смесям **STATS_SHARE** и парой CPU на каждый `@KafkaListener` concurrency) в **`k6/`** два удобных входа: **`lab13-k6-matrix.sh`** — **четыре** прогона из таблицы §0.8 (**`summary_CPU05_conc1.json`** … без **`_mix`**); **`lab13-k6-matrix-graphs.sh`** — **12** прогонов и имена **`summary_CPU05_conc1_mix50.json`** и т.д. (смеси **`STATS_SHARE`** как в LAB10/LAB8: **`0.05` / `0.5` / `0.95`** → **`_mix05`**, **`_mix50`**, **`_mix95`**). Внутри вызывается **`run-lab13-kafka-proxy-matrix.sh`**. Картинки **`reports-lab13/lab13_latency_vs_cpu.png`** (и **`…_conc1.png`**, **`…_conc2.png`**) строит **`plot_lab13_reports.py`** при полном наборе **`*_mix*.json`**. Переменная **`LAB13_PANEL_CONC`** задаёт дублирование в **`lab13_latency_vs_cpu.png`** (по умолчанию **`2`**).
